...
int	calculate_length_for_env_expansion(char *word, char **env_var, t_quote *state, int *length)
{
	int			env_var_index;
	int			y;
	int			word_Index;
	...
	else
	{
		...
		while (env_var[env_var_index])
		{
			...
			{
				word_Index = get_env_var_expansion_length(word, length, env_var[env_var_index], state);
				if (state->found == 1)
					return (word_Index);
			}
			env_var_index++;
