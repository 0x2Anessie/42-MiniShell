 ...
 *   est crucial pour les traitements ultérieurs, notamment dans le contexte d'un shell.
 *
 *   Importance de la distinction des sous-mots :
 *   - Précision de l'analyse : Permet de traiter correctement les commandes et arguments
 *     dans un shell, en respectant la présence de citations et de doubles citations.
 *   - Gestion correcte des espaces : Les espaces à l'intérieur des citations ne sont pas
 *     considérés comme des séparateurs de mots, ce qui nécessite une analyse plus fine.
 *
 * @valeur_retour: 
 ...
 *   t_quote state = {0, 0};
 *   int nombre_sous_mots = len_d_array(word, &state);
 *
 * @dépendances: 
 *   - update_quoting_state pour mettre à jour l'état des citations.
 *   - on_word pour déterminer si un caractère appartient à un nouveau sous-mot.
 *
 * @graphe_de_flux:
 *   Début
 *     |
 ...
 * @nom: split_word
 *
 * @description: Sépare un mot en plusieurs sous-mots en fonction des citations et
 *               des doubles citations.
 *
 * @paramètres:
 ...
 *   - Distinguer les sous-mots : Dans un contexte de shell, par exemple, les mots
 *     peuvent être séparés par des espaces, mais les espaces à l'intérieur des
 *     citations ne doivent pas être considérés comme des séparateurs.
 *   - Préserver l'intégrité des sous-mots : Les sous-mots à l'intérieur des citations
 *     doivent être traités comme des unités indivisibles, même s'ils contiennent des
 *     espaces ou d'autres caractères spéciaux.
 *
 * @valeur_retour: 
 *   Renvoie un tableau de chaînes de caractères contenant les sous-mots. Renvoie NULL
 ...
 *   t_quote state = {0, 0};
 *   char **mots = split_word(word, &state);
 *
 * @dépendances: 
 *   - ft_malloc_with_tracking pour l'allocation de mémoire.
 *   - len_d_array pour déterminer la taille du tableau.
 *   - malloc_word pour allouer chaque sous-mot.
 *   - update_quoting_state pour mettre à jour l'état des citations.
 *   - on_word pour vérifier si un caractère appartient à un nouveau sous-mot.
 *
 * @graphe_de_flux:
 *   Début
 *     |
 ...
 * @brève_description: Trouve la position de la prochaine guillemet simple dans une chaîne de commande.
 * 
 * @param cmd: Chaîne de caractères représentant la commande à analyser.
 * 
 * @description_détaillée:
 *   La fonction 'next_squote' parcourt la chaîne 'cmd' pour trouver la position de la prochaine guillemet simple (').
 *   Elle commence à l'index zéro et vérifie les conditions suivantes :
 *   - Si le premier caractère est un guillemet simple suivi immédiatement par un autre guillemet simple, elle retourne l'index actuel.
 *   - Sinon, elle parcourt la chaîne jusqu'à trouver un guillemet simple ou atteindre la fin de la chaîne.
 *   
 *   Pourquoi identifier la position de la guillemet simple ?
 *   - Parsing de commande : La détection des guillemets simples est essentielle pour interpréter correctement 
 *     les chaînes de caractères dans un interpréteur de commande, notamment pour gérer les chaînes littérales.
 *   - Gestion des chaînes : Permet de traiter ou de manipuler correctement des segments spécifiques d'une commande.
 *
 * @valeur_de_retour: 
 ...
 *   int position = next_squote(cmd); // Résultat attendu : position du premier guillemet simple après "echo "
 *
 * @dépendances: Aucune.
 *
 * @graphe_de_flux:
 ...
 * @brève_description: Trouve la position de la prochaine guillemet double dans une chaîne de commande.
 * 
 * @param cmd: Chaîne de caractères représentant la commande à analyser.
 * 
 * @description_détaillée:
 *   La fonction 'next_dquote' parcourt la chaîne 'cmd' pour trouver la position de la prochaine guillemet double (").
 *   Elle commence à l'index zéro et vérifie les conditions suivantes :
 *   - Si le premier caractère est un guillemet double suivi immédiatement par un autre guillemet double, elle retourne l'index actuel.
 *   - Sinon, elle parcourt la chaîne jusqu'à trouver un guillemet double ou atteindre la fin de la chaîne.
 *   
 *   Pourquoi identifier la position de la guillemet double ?
 *   - Parsing de commande : La détection des guillemets doubles est essentielle pour interpréter correctement 
 *     les chaînes de caractères dans un interpréteur de commande, notamment pour gérer les chaînes littérales.
 *   - Gestion des chaînes : Permet de traiter ou de manipuler correctement des segments spécifiques d'une commande.
 *
 * @valeur_de_retour: 
 ...
 * @exemples_d'utilisation:
 *   char *cmd = "echo \"Hello, world!\"";
 *   int position = next_dquote(cmd); // Résultat attendu : position du premier guillemet double après "echo "
 *
 * @dépendances: Aucune.
 *
 * @graphe_de_flux:
 ...
 * @brève_description: Calcule la longueur d'un segment de commande jusqu'à la prochaine guillemet simple (').
 * 
 * @param cmd: Chaîne de caractères représentant la commande à analyser.
 ...
 * 
 * @description_détaillée:
 *   La fonction 'next_squote_len' parcourt la chaîne 'cmd' à partir de 'index', cherchant la fin du segment
 *   actuel qui est délimité par une guillemet simple. Elle met à jour l'index pour pointer après la fin de 
 ...
 *   - Gestion des états de guillemets : Les guillemets simples affectent la manière dont les commandes sont 
 ...
 *   t_quote state;
 *   int new_index = next_squote_len(cmd, index, &state); // Résultat attendu : index après "world"
 *
 * @dépendances:
 ...
 *   |
 *   v
 * Parcourir la chaîne 'cmd' jusqu'à trouver un guillemet simple ou la fin de la chaîne
 *   |
 *   v
