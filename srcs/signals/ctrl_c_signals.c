...
/*   Updated: 2023/11/30 02:28:25 by raveriss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 *     généraux du programme, en dehors de situations spécifiques comme les 'here documents'.
 *
 * @dépendances:
 ...
 * @dépendances:
 *   - g_all: Structure globale contenant l'état du programme.
 *   - close: Fonction de la bibliothèque standard pour fermer un descripteur de fichier.
...
void	ctrl_c_handler_here_doc(int sig)
{
	(void)sig;
	close(0);
	write(STDERR_FILENO, FT_NEWLINE, ft_strlen(FT_NEWLINE));
 ...
 *   - Si 'is_here_doc' est vrai, configure le signal CTRL_C pour utiliser le 
 *     gestionnaire ctrl_c_handler_here_doc, spécifique au contexte 'here document'.
 *   - Si 'is_here_doc' est faux, configure le signal CTRL_C pour utiliser le 
 *     gestionnaire ctrl_c_handler standard et le signal CTRL_BACKSLSH pour 
 ...
 *
 * @erreurs_et_effets_de_bord:
 *   - L'installation des gestionnaires de signaux peut échouer, mais cette fonction 
 *     ne gère pas ces éventualités.
 *
 * @exemples_d'utilisation:
 *   - Appelée pour configurer les gestionnaires de signaux en fonction du contexte, 
 *     notamment lors de l'entrée et de la sortie des 'here documents'.
 *
 * @dépendances:
 *   - g_all: Structure globale contenant l'état du programme.
 *   - signal: Fonction standard C pour la gestion des signaux.
 *   - ctrl_c_handler_here_doc: Gestionnaire spécifique pour SIGINT dans le contexte 
 *     'here document'.
 ...
 *     |           \
 *     v            v
 *   Configurer     Configurer CTRL_C avec ctrl_c_handler
 *   CTRL_C avec    Configurer CTRL_BACKSLSH avec IGNORE_SIG_ACTION
...
void	handle_sig(void)
{
	if (g_all.utils->is_here_doc)
		signal(CTRL_C, &ctrl_c_handler_here_doc);
	else
	{
		signal(CTRL_C, &ctrl_c_handler);
