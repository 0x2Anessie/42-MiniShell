 ...
 * 
 * Cette fonction met à zéro les indicateurs d'état des guillemets simples et
 * doubles dans une structure 't_quote' fournie. Elle est utilisée pour
 * réinitialiser l'état avant de commencer le traitement d'une nouvelle chaîne
 * ou d'un nouveau mot.
 * 
 * @param state: Pointeur sur la structure 't_quote' dont l'état doit être
 * réinitialisé.
 * 
 * @erreur: Aucune erreur définie pour cette fonction.
 * 
 * @effet_de_bord: Réinitialise les champs 'doubl_quot_status' et
 * 'singl_quot_status' de la structure pointée par 'state'.
 * 
 * @exemple: reset_quoting_state(state) réinitialise l'état des guillemets de
 * 'state'.
 * 
 ...
 *
 * @pourquoi:
 * - Précision d'Allocation : Permet de déterminer la taille exacte nécessaire
 * pour une chaîne de caractères représentant un nombre, assurant une
 * allocation de mémoire efficace.
 * - Utilité dans la Conversion : Fournit une information essentielle pour la
 * conversion de nombres en chaînes, évitant les erreurs de dépassement de
 * mémoire.
 *
 * @valeur_de_retour:
 * Retourne le nombre de chiffres nécessaires pour représenter 'n' en tant que
 * chaîne.
 *
 ...
 * long nombre = 12345;
 * int longueur = length_of_number_for_string_conversion(nombre);
 * ------------> longueur est 5
 *
 * long nombre_zero = 0;
 * int longueur_zero = length_of_number_for_string_conversion(nombre_zero);
 * --------------> longueur_zero est 1
 *
 * @dependances: Aucune.
 *
 ...
 * nombres négatifs.
 *
 * @param res: char *res, pointeur initial vers la chaîne de caractères
 * résultante (non utilisé).
 * @param n: long n, le nombre négatif à convertir.
 * @param length: int length, la longueur calculée de la représentation
 * numérique, sans compter le signe moins.
 *
 * @pourquoi:
 * - Représentation Correcte des Négatifs : Assure que les nombres négatifs
 * sont représentés correctement avec un signe moins, conformément aux
 * conventions numériques.
 * - Modularité et Clarté : Sépare la logique de traitement des nombres
 * négatifs de celle des nombres positifs, rendant le code de 'ft_itoa' plus
 * clair et modulaire.
 *
 * @valeur_de_retour:
 * Retourne un pointeur vers la nouvelle chaîne de caractères représentant le
 * nombre négatif. En cas d'échec d'allocation, retourne NULL.
 *
 * @erreurs_possibles_et_effets_de_bord: 
 * - Si l'allocation de mémoire échoue, la fonction retourne NULL.
 * - La chaîne retournée doit être libérée par l'appelant pour éviter les
 * fuites de mémoire.
 *
 * @exemples_utilisation:
 * long num_neg = -123;
 ...
 *
 * @dependances: 
 * - ft_malloc_with_tracking pour allouer la mémoire pour la chaîne de
 * caractères.
 *
 * @graphe_de_flux:
 *   Début
 ...
 *
 * @pourquoi:
 * - Transformation de Données : Permet de convertir les données numériques en
 * format textuel, facilitant leur manipulation dans divers contextes comme
 * l'affichage ou la sauvegarde.
 * - Compatibilité avec les Opérations de Chaînes : Rend possible l'utilisation
 * de nombres dans des contextes où les chaînes de caractères sont nécessaires
 * ou plus pratiques.
 *
 * @valeur_de_retour:
 * Retourne un pointeur vers la chaîne de caractères représentant le nombre
 * 'n'. En cas d'échec d'allocation, retourne NULL.
 *
 * @erreurs_possibles_et_effets_de_bord: 
 * - Si l'allocation de mémoire échoue, la fonction retourne NULL.
 * - La chaîne retournée doit être libérée par l'appelant pour éviter les
 ...
 *
 * @dependances: 
 * - length_of_number_for_string_conversion pour calculer la longueur du
 * nombre.
...
char	*convert_int_to_string_with_tracking(t_data *data, int n)
{
	...
	res = ft_malloc_with_tracking(data, sizeof(char) * (index + sizeof('\0')));
	res[index] = '\0';
	if (!res)
		return (NULL);
	index--;
 ...
 *
 * @pourquoi:
 * - Validation de Chiffres : Permet de vérifier si un caractère fait partie
 * des chiffres décimaux, ce qui est utile dans les contextes de validation de
 * données numériques, de parsing ou de manipulation de chaînes de caractères.
 * - Universel et Réutilisable : Fournit une implémentation standardisée et
 * facilement réutilisable pour vérifier les caractères numériques, réduisant
 * ainsi la redondance du code.
 *
 * @valeur_de_retour:
 ...
 * @exemples_utilisation:
 * char c1 = '5';
 * int result1 = is_decimal_digit(c1); // Retourne 1 (vrai)
 *   
 * char c2 = 'a';
 * int result2 = is_decimal_digit(c2); // Retourne 0 (faux)
 *
 * @dependances: Aucune.
 *
 * @graphe_de_flux:
